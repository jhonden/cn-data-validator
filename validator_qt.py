            ...
            if package_type == 'NIC Package':
                ...
                if nic_validation:
                    if not nic_validation.get('valid', True):
                        # NIC 校验失败
                                                                                                                                                # 对于完全不支持的包，需要跳过 issues 收集，但 issues 变量可能未定义                                                                                                                                                if not all_unsupported:                                                                                                                                                    issues = []                        # 检查是否完全不支持的包                        all_unsupported = nic_validation.get("all_unsupported", False)                        if all_unsupported:                            # 完全不支持的包：显示为 Invalid 状态                            status = "Invalid"                            status_color = "#F44336"  # Red                            detail = "Invalid NIC package: No supported network elements found, no need to collect"                        else:                            # 部分不支持的包：显示为 Warning 状态                            status = "Warning"                            status_color = "#FF9800"  # Orange                        status = 'Warning'
