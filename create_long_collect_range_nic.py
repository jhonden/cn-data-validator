...
def create_long_collect_range_package(output_path='test_data/test_long_collect_range.tar.gz'):
    ...
    # 创建临时目录
    temp_dir = tempfile.mkdtemp(prefix='long_collect_')
    print(f"Creating long collect range NIC package in: {temp_dir}")
...
<taskInfo taskType="sceneTask" taskExecType="instance" sellNEVer="...">
    ...
    print(f"✓ Created: taskparam/TaskExtValue.xml (CollectRange: 168h = 7 days)")
...
if __name__ == '__main__':
    print("=" * 70)
    print("Create Long Collect Range NIC Package")
    print("=" * 70)
    print()
    print("This package has a collection time range of 168h (7 days, > 24h).")
    print("Expected validation result:")
    print("  Status: Valid (Green)")
    print("  No errors related to collection time range")
    print()
    print("=" * 70)
    print()

    # 创建测试包
    output_file = create_long_collect_range_package()
    ...
    print(f"4. Look for 'test_long_collect_range.tar.gz' in the table")
    print(f"5. Verify Status is 'Valid' (Green)")
    print(f"6. Check that no collection time range errors are shown")
    print()
