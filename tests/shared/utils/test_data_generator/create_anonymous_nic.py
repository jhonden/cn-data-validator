#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""创建匿名化方式采集的 NIC 包测试数据

预期结果:
- 状态：Invalid (红色)
- 提示：The NIC package was collected in anonymous mode, cannot meet network
        assessment requirements. Please re-collect in non-anonymous mode.
"""
...
def create_anonymous_package(output_path='tests/data/test_data/test_anonymous.tar.gz'):
    """创建匿名化采集的 NIC 包
    ...
    # 创建临时目录
    temp_dir = tempfile.mkdtemp(prefix='anonymous_')
    print(f"Creating anonymous NIC package in: {temp_dir}")
...
<taskInfo taskType="sceneTask" taskExecType="instance" selNEInfo="64:V500R023C10SPC110|56:V500R023C10SPC100|9596:V500R023C10SPH156" selProductType="" isAllVers="false">
    ...
    <TaskDescription>
        ...
        <AnonymousAuthMode>true</AnonymousAuthMode>
    ...
    print(f"✓ Created: taskparam/TaskExtValue.xml (AnonymousAuthMode: true)")
...
if __name__ == '__main__':
    print("=" * 70)
    print("Create Anonymous Mode NIC Package")
    print("=" * 70)
    print()
    print("This package was collected in anonymous mode (AnonymousAuthMode=true).")
    print("Expected validation result:")
    print("  Status: Invalid (Red)")
    print("  Message: The NIC package was collected in anonymous mode, cannot meet")
    print("           network assessment requirements. Please re-collect in")
    print("           non-anonymous mode.")
    print()
    print("=" * 70)
    print()

    # 创建测试包
    output_file = create_anonymous_package()
    ...
    print(f"4. Look for 'test_anonymous.tar.gz' in the table")
